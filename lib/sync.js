...
import { runPublishedBetaBuildExpiry } from './expire.js';
...
export async function runReleaseSync(
  ...
  github,
  DRY_RUN,
  triggerNextBuild = true,
  {
    onVersionReleased = async () => {},
    publishedBetaBuilds = null,
  } = {},
) {
  log('--- Release Sync ---');
  ...
  if (publishedBetaBuilds?.workflowId) {
    await runPublishedBetaBuildExpiry(asc, DRY_RUN, {
      liveVersion: liveStatus.version,
      workflowId: publishedBetaBuilds.workflowId,
      betaBranch: publishedBetaBuilds.betaBranch,
    });
  }
