...
const MODES = new Set(['all', 'deploy', 'sync', 'expire', 'trigger', 'notes', 'release-pr', 'rebase-prs', 'build-status']);
...
export function parseCliArgs(args, rootDir, environment = process.env) {
  ...
  if (!MODES.has(mode)) {
    throw new Error(`Unknown mode: ${mode}. Expected all, deploy, sync, expire, trigger, notes, release-pr, rebase-prs, or build-status.`);
  }
