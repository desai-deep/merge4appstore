...
export { tryAcquireProcessLock } from './process-lock.js';
...
export class ReleaseClient {
  ...
  async source(selection) {
    ...
    return { commitSha: data.head?.sha || data.sha, branch: data.head?.ref || required(selection.branch, 'Branch') };
  }
  async ownership(selection, { requireInternal = false } = {}) {
    const products = await this.catalogue();
    const product = products.find(p => p.appId === selection.appId && p.workflows.some(w => w.id === selection.workflowId && (!requireInternal || w.enabled && w.internalOnly)));
  ...
  async validate(selection) {
    const product = await this.ownership(selection, { requireInternal: true });
  ...
  async trigger(selection) {
    let verified;
    try { verified = await this.validate(selection); }
    catch (error) { error.beforeBuildStart = true; throw error; }
    if (selection.commitSha && selection.commitSha !== verified.commitSha) {
      const error = new Error('Source changed since validation. Validate and save the app again before starting a build.');
      error.code = 'SOURCE_CHANGED';
      error.beforeBuildStart = true;
      throw error;
    }
    return this.provider.trigger({ ...verified, purpose: 'internal_testing', provider: 'xcode_cloud' });
  }
  async status(runId, selection) {
    await this.ownership(selection);
    ...
    const commitSha = typeof run.attributes.sourceCommit === 'string' ? run.attributes.sourceCommit : run.attributes.sourceCommit?.commitSha;
    const sourceMismatch = Boolean(selection.commitSha && commitSha && selection.commitSha !== commitSha);
    return { runId, number: run.attributes.number, progress: run.attributes.executionProgress,
      completionStatus: sourceMismatch ? 'SOURCE_MISMATCH' : run.attributes.completionStatus,
      providerCompletionStatus: run.attributes.completionStatus, sourceMismatch,
      expectedCommitSha: selection.commitSha || null, commitSha, builds: result };
  }
}
