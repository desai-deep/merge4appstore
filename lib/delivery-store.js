...
export class MemoryDeliveryStore {
    ...
    for (const [key, receipt] of this.receipts) {
      if (
        ...
        || (
          ...
          && !(releaseReason !== null && receipt.retryReason === releaseReason)
        )
      ) continue;
  ...
  async inspectFailedReceipts() {
    ...
    for (const [key, receipt] of this.receipts) {
      ...
      failed.push({
        ...
        revision: deliveryReceiptRevision(receiptHash(key), receipt),
  ...
  async retireFailed({
    ...
    await this.inspectFailedReceipts();
    // Rebuild after the asynchronous inspection boundary so a concurrent
    // release cannot turn a stale selection into a terminal acknowledgement.
    const failedByHash = new Map();
    for (const [key, receipt] of this.receipts) {
      if (receipt.state === 'failed') {
        failedByHash.set(receiptHash(key), { key, receipt });
    ...
    const selected = failedByHash.get(hash);
    if (!selected) {
      const error = new Error(
        `Receipt is no longer a readable failed delivery: ${hash}`,
      );
      error.code = 'ENOTFAILED';
      throw error;
    }
    const currentRevision = deliveryReceiptRevision(hash, selected.receipt);
    ...
    const now = this.now();
    const { expiresAt: _expiresAt, ...receipt } = selected.receipt;
    this.receipts.set(selected.key, {
      ...receipt,
      token: crypto.randomUUID(),
      state: 'retired',
      ownerPid: null,
      ...
      retirement: {
        ...
        failedAt: receipt.updatedAt,
        attempts: receipt.attempts,
        cursor: Number(receipt.cursor || 0),
        lastError: typeof receipt.lastError === 'string' ? receipt.lastError : null,
      },
    });
    return true;
