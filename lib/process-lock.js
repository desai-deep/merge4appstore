...
function lockHelperEnvironment(environment = process.env) {
  const allowed = new Set([
    'PATH', 'LANG', 'TERM', 'NO_COLOR', 'TMPDIR', 'TMP', 'TEMP',
  ]);
...
function validateHolderCommand(holderCommand) {
  if (
    !Array.isArray(holderCommand)
    || holderCommand.length === 0
    || typeof holderCommand[0] !== 'string'
    || holderCommand[0].length === 0
    || Array.from(holderCommand).some(argument => (
      typeof argument !== 'string' || argument.includes('\0')
    ))
  ) {
    const error = new TypeError('Process-lock holder command must be a non-empty argv array');
    error.code = 'ELOCKCONFIG';
    throw error;
  }
  return holderCommand;
}

function fileLockCommand(lockFile, holderCommand = ['/bin/sh', '-c', FILE_LOCK_HELPER]) {
  const command = validateHolderCommand(holderCommand);
