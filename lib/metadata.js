...
function repositoryMetadataEntries(github, metadataPath, ref) {
...
export async function getManagedWhatsNewLocales(asc, github, {
  metadataPath,
  ref,
} = {}) {
  if (!metadataPath) return new Set();

  const { entries, root } = repositoryMetadataEntries(github, metadataPath, ref);
  const managed = new Set();
  let managesDefaultLocale = false;

  for (const entry of entries) {
    if (entry.type !== 'blob') continue;
    const relative = entry.path.slice(root.length + 1);
    const parts = relative.split('/');
    if (parts.length === 1 && parts[0] === 'whats_new.txt') {
      managesDefaultLocale = true;
    } else if (parts.length === 2 && parts[1] === 'whats_new.txt') {
      managed.add(parts[0]);
    }
  }

  if (managesDefaultLocale) managed.add(await asc.getAppPrimaryLocale());
  return managed;
}
...
async function syncReviewMetadata(asc, versionId, attributes, dryRun) {
  ...
  if (!existing) {
    const required = {
      contactFirstName: 'review/contact_first_name.txt',
      contactLastName: 'review/contact_last_name.txt',
      contactPhone: 'review/contact_phone.txt',
      contactEmail: 'review/contact_email.txt',
    };
    const missing = Object.entries(required)
      .filter(([field]) => !reviewAttributes[field])
      .map(([, file]) => file);
    if (missing.length > 0) {
...
export async function syncLocalizedMetadata(asc, github, {
  ...
  const { entries } = repositoryMetadataEntries(github, metadataPath, ref);
