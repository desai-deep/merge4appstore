...
const SCREENSHOT_TYPES = new Set([
  ...
  'APP_IPAD_105', 'APP_IPAD_97', 'APP_WATCH_ULTRA', 'APP_WATCH_SERIES_10',
  'APP_WATCH_SERIES_7', 'APP_WATCH_SERIES_4', 'APP_WATCH_SERIES_3',
...
export function discoverLocalizedMetadata(entries, metadataRoot = 'AppStore', readBlob) {
  ...
  for (const entry of entries) {
    ...
    if (entry.type === 'tree' && parts.length === 3) {
      if (kind === 'screenshots' && !SCREENSHOT_TYPES.has(candidate)) {
        throw new Error(`Unknown screenshot display type directory: ${entry.path}`);
      }
      sets[candidate] ||= [];
      continue;
    }
