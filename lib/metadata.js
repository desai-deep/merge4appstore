...
export function discoverLocalizedMetadata(entries, metadataRoot = 'AppStore', readBlob) {
  ...
  for (const entry of entries) {
    ...
    sets[candidate] ||= [];
    sets[candidate].push(mediaAsset(entry, fileName, parts.slice(2).join('/'), bytes));
  }
  ...
  return { versionAttributes, reviewAttributes, localizations };
}
...
export async function syncLocalizedMetadata(asc, github, {
  ...
  await syncAppInfoMetadata(asc, metadata.localizations, dryRun);
  ...
  const hasLocalizedVersionMetadata = Object.values(metadata.localizations).some(value => (
