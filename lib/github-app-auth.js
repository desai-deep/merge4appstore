...
export class GitHubAppAuthenticator {
  ...
  async discoverRepositoryInstallation(
    ...
    const request = this.request(installationPath)
      .then(({ data }) => {
        ...
        if (this.installationsInFlight.get(cacheKey) === request) {
          this.installationsInFlight.delete(cacheKey);
        }
      });
  ...
  invalidateRepositoryInstallation(owner, repository, { repositoryId = null } = {}) {
    ...
    // Keep active requests owned until they settle, including rediscovery.
    // Dropping them here would let concurrent callers mint duplicate tokens.
  }
  ...
  async installationToken(owner, repository, { repositoryId = null, signal = null } = {}) {
    ...
    if (this.inFlight.has(repositoryScope)) {
      return waitForSignal(this.inFlight.get(repositoryScope), signal);
    }
    ...
    const request = (async () => {
      ...
      if (this.inFlight.get(repositoryScope) === request) this.inFlight.delete(repositoryScope);
    });
    this.inFlight.set(repositoryScope, request);
    return waitForSignal(request, signal);
  }
