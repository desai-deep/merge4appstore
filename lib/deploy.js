...
export function submissionFailureComment({ buildNumber, version, versionId, buildSelected, error }) {
  ...
  return `${marker}\nBuild #${buildNumber} could not be submitted to App Store review.\n\n**App Store Connect requirements**\n${details}\n\n${buildState}Fix the App Store Connect requirements, then rerun deployment reconciliation. A new build is not required unless App Store Connect reports a build-specific problem.`;
}
    ...
    try {
      ...
      if (prNumber) {
        ...
        if (result) {
          ...
          if (error.reviewSubmissionId) {
            try {
              const cleanup = await asc.deleteDraftReviewSubmission(error.reviewSubmissionId);
              if (cleanup.deleted) {
                log(`Removed failed draft review submission ${error.reviewSubmissionId}`);
              } else {
                log(`Kept review submission ${error.reviewSubmissionId} because its state is ${cleanup.state}`);
              }
            } catch (cleanupError) {
              log(`Warning: Could not remove failed draft review submission ${error.reviewSubmissionId}: ${cleanupError.message}`);
            }
          }
        } else {
