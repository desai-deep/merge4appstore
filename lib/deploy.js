...
async function surfaceBlockedRelease(asc, github, blockedStatus, blockReason) {
  try {
    ...
    if (commitInfo?.found && commitInfo.commitSha) {
      prNumber = github.findPRFromCommit(commitInfo.commitSha);
    }
...
export async function recoverMissedProductionBuild(asc, github, DRY_RUN) {
  ...
  try {
    await github.refreshEnvironment?.();
    const commitSha = github.getProductionHead({ strict: true });
    if (!commitSha) {
      throw recoveryUnavailable('Could not resolve the production branch head');
    }

    const prNumber = github.findPRFromCommit(commitSha, { strict: true });
    if (!prNumber) {
