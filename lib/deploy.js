...
export async function runDeployCheck(asc, github, DRY_RUN) {
  ...
  if (rejectedStatus.rejected) {
    const rejectedBuildNum = parseInt(rejectedStatus.buildNumber, 10);
    const latestBuildNum = parseInt(latestBuild.buildNumber, 10);

    if (isNaN(rejectedBuildNum) || isNaN(latestBuildNum)) {
      log('Rejected or candidate build number is non-numeric, skipping to avoid resubmitting a rejected build');
      return;
    }

    if (latestBuildNum <= rejectedBuildNum) {
      log(
        `Latest eligible build #${latestBuild.buildNumber} is not newer than rejected build #${rejectedStatus.buildNumber} - waiting for the next Xcode Cloud build`
      );
      return;
    }
  }
