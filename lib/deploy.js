...
export async function runDeployCheck(asc, github, DRY_RUN) {
  ...
  // App Store Connect only allows one open review submission per app. Clear any
  // blocking ones first (empty drafts left by a failed run, an UNRESOLVED_ISSUES
  // submission from a prior rejection, or an in-flight review being superseded)
  // so submitForReview doesn't fail with HTTP 409.
  try {
    const cleared = await asc.clearBlockingReviewSubmissions();
    for (const c of cleared) {
      log(`Cleared blocking review submission ${c.id} (${c.state}) - ${c.action}${c.error ? ': ' + c.error : ''}`);
    }
  } catch (e) {
    log(`Warning: Could not clear blocking review submissions: ${e.message}`);
  }
