...
export function jobsForGitHubEvent(profile, event, payload, deliveryId) {
  ...
  if (configuredRepositoryId && payloadRepositoryId) {
  ...
  if (event === 'pull_request') {
    ...
    const isReleaseTrackPullRequest = pull.base?.ref === productionBranch
      && pull.head?.ref === betaBranch;
    const isReleasePullRequest = releasePullRequest.enabled && isReleaseTrackPullRequest;
    const isBuildPullRequest = !isReleasePullRequest;
    const releaseNotesJob = () => {
      if (!payload.changes?.title
        || !isReleasePullRequest
        || pull.merged !== true
        || !pull.merge_commit_sha) return null;
      return {
        mode: 'release-notes',
    ...
    if (action === 'closed' && isBuildPullRequest) {
    ...
    if (action === 'edited' && payload.changes?.body
      ...
      const releaseJob = releaseNotesJob();
      if (releaseJob) jobs.push(releaseJob);
      return jobs;
    }
...
export function jobsForXcodeCloudEvent(profile, payload) {
  ...
  if (status !== 'SUCCEEDED') return [statusJob];

  const jobs = [statusJob, {
    mode: 'notes',
    purpose,
    runId,
    commitSha: statusJob.commitSha,
    deliveryId: runId,
  }];
  if (purpose === 'production') jobs.push({ mode: 'deploy', deliveryId: runId });
  return jobs;
}
...
export function webhookSettings(profile, environment = process.env) {
  ...
  const versionTokenEnv = profile.ci?.version?.token_env
  ...
  return {
    ...
    versionTokenEnv,
    versionToken: environment[versionTokenEnv] || '',
  };
}
