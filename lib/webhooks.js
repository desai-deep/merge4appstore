...
import { resolveAutoRebasePullRequests, resolveReleasePullRequest } from './profile.js';
...
export function jobsForGitHubEvent(profile, event, payload, deliveryId) {
  ...
  if (event === 'pull_request') {
    ...
    const baseChangedToBeta = action === 'edited'
      && payload.changes?.base
      && pull.base?.ref === (profile.repository.beta_branch || 'develop');
    if ((['opened', 'reopened', 'synchronize'].includes(action) || baseChangedToBeta)
      && pull.base?.ref === (profile.repository.beta_branch || 'develop')) {
      jobs.push({
        mode: 'trigger',
        purpose: 'pull_request',
  ...
  if (event === 'push' && payload.deleted !== true) {
    ...
    if (purpose && payload.after && !/^0+$/.test(payload.after)) {
      ...
      else jobs.push({ mode: 'trigger', purpose, commitSha: payload.after, branch, deliveryId });
    }
  }
