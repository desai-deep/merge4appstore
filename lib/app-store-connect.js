...
export function appStoreErrorDetails(responseBody = {}) {
  ...
  const associatedErrors = errors.flatMap(error => (
    Object.values(error?.meta?.associatedErrors || {}).flat()
  ));
  const source = associatedErrors.length > 0 ? associatedErrors : errors;
...
export class AppStoreConnectAPI {
  ...
  async deleteDraftReviewSubmission(submissionId) {
    ...
    if (state !== 'READY_FOR_REVIEW') {
      return { deleted: false, state: state || 'UNKNOWN' };
    }
