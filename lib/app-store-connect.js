...
export class AppStoreConnectAPI {
  ...
  async cancelReview(versionId) {
    ...
    await this.cancelReviewSubmissionById(submissionId);
    return { success: true };
  }

  async cancelReviewSubmissionById(submissionId) {
  ...
  async deleteReviewSubmission(submissionId) {
    await this.request(`/reviewSubmissions/${submissionId}`, { method: 'DELETE' });
  }

  // App Store Connect allows only one open review submission per app, so a stale
  // or half-created submission blocks new ones (HTTP 409 "cannot be reviewed").
  // Before creating a fresh submission, clear anything still open: empty drafts
  // are deleted; submissions that carry items (e.g. UNRESOLVED_ISSUES from a
  // prior rejection, or an in-flight review being superseded) are canceled.
  // COMPLETE submissions are finished and never block, so they're left alone.
  async clearBlockingReviewSubmissions(exceptId = null) {
    const appId = await this.getAppId();
    const data = await this.request(
      `/reviewSubmissions?filter[app]=${appId}&include=items&limit=200&limit[items]=50&fields[reviewSubmissionItems]=state`
    );
    ...
    for (const submission of data.data || []) {
      ...
      if (exceptId && submission.id === exceptId) continue;

      const itemCount = (submission.relationships?.items?.data || []).length;
      try {
        if (itemCount === 0) {
          await this.deleteReviewSubmission(submission.id);
          cleared.push({ id: submission.id, state, action: 'deleted' });
        } else {
          await this.cancelReviewSubmissionById(submission.id);
          cleared.push({ id: submission.id, state, action: 'canceled' });
        }
      } catch (e) {
        cleared.push({ id: submission.id, state, action: 'failed', error: e.message });
      }
    }
  ...
  async getOrCreateDraftReviewSubmission(versionId) {
    ...
    try {
      ...
      // Attaching the version failed - roll back the now-empty submission so it
      // doesn't linger as a stuck draft and block the next attempt.
      try {
        await this.deleteReviewSubmission(submissionId);
        log(`Rolled back empty review submission ${submissionId} after item creation failed`);
      } catch (cleanupError) {
        log(`Warning: failed to roll back review submission ${submissionId}: ${cleanupError.message}`);
      }
      throw e;
    }
