...
export class AppStoreConnectAPI {
  ...
  async findAppStoreVersionLocalization(versionId, locale) {
    ...
    return (response.data || []).find(item => item.attributes?.locale === locale) || null;
  }
  ...
  async syncScreenshotSet(localizationId, displayType, desired, dryRun = false) {
    ...
    if (dryRun) {
      return { kept: desired.length - missing.length, uploaded: missing.length, removed: remaining.length };
    }
    ...
    await this.replaceScreenshotOrder(set.id, slots.map(slot => slot.id));
    return { kept: desired.length - missing.length, uploaded: missing.length, removed: remaining.length };
  }
