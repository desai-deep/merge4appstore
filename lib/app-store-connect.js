...
export class AppStoreConnectAPI {
  ...
  async getLatestTestFlightReadyBuild(excludeBuildIds = []) {
    const appId = await this.getAppId();
    ...
    for (const build of data.data || []) {
      ...
      if (excludedBuildIds.has(build.id)) continue;
  ...
  async submitForReview(versionId) {
    try {
      ...
      const deprecatedSubmissionBlocked =
        error.message.includes("The resource 'appStoreVersionSubmissions' does not allow 'CREATE'");
      if (!deprecatedSubmissionBlocked) {
        throw error;
      }
    }
  ...
  async getOrCreateDraftReviewSubmission(versionId) {
    const appId = await this.getAppId();
    const states = ['READY_FOR_REVIEW', 'WAITING_FOR_REVIEW', 'IN_REVIEW', 'UNRESOLVED_ISSUES', 'CANCELING', 'COMPLETING'];
    const filterStates = encodeURIComponent(states.join(','));
    const data = await this.request(
      `/reviewSubmissions?filter[app]=${appId}&filter[state]=${filterStates}&include=items&limit=200&limit[items]=50&fields[reviewSubmissionItems]=appStoreVersion,state`
    );
