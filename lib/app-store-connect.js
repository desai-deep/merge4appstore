...
export class AppStoreConnectAPI {
  ...
  async getPublishedWorkflowCommits(workflowId, limit = 50) {
    ...
    const builds = await this.request(
      `/builds?filter[app]=${appId}&sort=-uploadedDate&limit=200&include=preReleaseVersion&fields[builds]=version,uploadedDate,preReleaseVersion&fields[preReleaseVersions]=version`
    );
    const commits = [];
    const seen = new Set();
