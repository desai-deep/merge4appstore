...
export class AppStoreConnectAPI {
  ...
  async getWorkflows(productId) {
    const workflows = [];
    let endpoint = `/ciProducts/${productId}/workflows`;
    const seen = new Set();
    while (endpoint) {
      if (seen.has(endpoint)) throw new Error('Apple workflow pagination repeated a page');
      seen.add(endpoint);
      ...
      if (url.origin !== 'https://api.appstoreconnect.apple.com' || !url.pathname.startsWith('/v1/')) throw new Error('Invalid Apple workflow pagination URL');
      endpoint = url.pathname.slice(3) + url.search;
    }
    return workflows;
  }
  ...
  async getBuildRunNotesContext(runId) {
    const [runData, buildsData] = await Promise.all([
      this.request(
        `/ciBuildRuns/${runId}?include=sourceBranchOrTag,pullRequest,workflow&fields[ciBuildRuns]=number,sourceCommit,isPullRequestBuild,executionProgress,completionStatus,sourceBranchOrTag,pullRequest,workflow&fields[scmGitReferences]=name,canonicalName&fields[scmPullRequests]=number,sourceBranchName,destinationBranchName&fields[ciWorkflows]=name`
      ),
      this.request(
        `/ciBuildRuns/${runId}/builds?limit=200&fields[builds]=version,processingState`
      ),
    ]);
    const run = runData.data;
    if (!run) throw new Error(`Xcode Cloud build run ${runId} was not found`);

    const pullRequestId = run.relationships?.pullRequest?.data?.id;
    const pullRequest = (runData.included || []).find(
      item => item.type === 'scmPullRequests' && item.id === pullRequestId
    );
    ...
    return {
      runId: run.id,
      workflowId: run.relationships?.workflow?.data?.id || null,
      number: run.attributes?.number || null,
      completionStatus: run.attributes?.completionStatus || null,
      commitSha: typeof sourceCommit === 'string'
        ? sourceCommit
        : sourceCommit?.commitSha
          || sourceCommit?.hash
          || sourceCommit?.canonicalHash
          || sourceCommit?.id
          || null,
      branch: sourceBranchForRun(run, runData)?.replace(/^refs\/heads\//, '') || null,
      targetBranch: pullRequest?.attributes?.destinationBranchName || null,
      pullRequest: pullRequest?.attributes?.number
        ? String(pullRequest.attributes.number)
        : null,
      builds: (buildsData.data || []).map(item => ({
        buildId: item.id,
        buildNumber: item.attributes?.version || null,
  ...
  async getPublishedWorkflowCommits(workflowId, limit = 200) {
    ...
    for (const build of builds.data || []) {
      ...
      let source = await this.getBuildSource(build.id, null, buildRuns);
      if (!source.found && build.attributes?.version != null) {
        // Apple sometimes omits the run's builds relationship. Run numbers are
        // workflow-scoped, so only fall back when the uploaded build number is
        // unique across every workflow configured for this app.
        allBuildRuns ||= await this.loadCIBuildRuns();
        source = await this.getBuildSource(
          build.id,
  ...
  async getBuildsForWorkflowCommit(workflowId, commitSha) {
    ...
    // Preserve the cheap workflow-local exact lookup above. Only enumerate the
    // app's other workflows when Apple omitted every exact build relationship.
    const allBuildRuns = await this.loadCIBuildRuns();
    for (const build of builds.data || []) {
      const source = await this.getBuildSource(
        build.id,
        build.attributes?.version,
        allBuildRuns,
      );
