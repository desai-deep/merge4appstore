...
export function formatTestFlightNotes(subjects, maxLength = 4000) {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    const candidate = [...included, lines[index], suffix].join('\n');
...
export async function generateTestFlightNotes({ build, payload, asc, github }) {
  ...
  const manualNotes = build.purpose === 'pull_request' ? pullRequest?.body?.trim() || '' : '';
  let subjects = [];
  ...
  if (build.includeCommits) {
    ...
    const prefix = [manualNotes, heading].filter(Boolean).join('\n\n');
    const available = Math.max(1, 4000 - (prefix ? prefix.length + 2 : 0));
    const commits = formatTestFlightNotes(subjects, available);
    generated = [prefix, commits.text].filter(Boolean).join('\n\n');
    ...
    const summary = pullRequest?.title
      || github.findPullRequestTitleForCommit(payload.commit, payload.branch)
      || commitSubject
