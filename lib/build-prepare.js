...
export async function generateTestFlightNotes({ build, payload, asc, github }) {
  ...
  let generated;
  if (build.includeCommits) {
    const heading = commitRange?.baseBuildNumber
      ? `Commits since build #${commitRange.baseBuildNumber}:`
      : firstPullRequestBuild
        ? 'Commits in this pull request:'
        : manualNotes ? 'Commits since the last published build:' : '';
