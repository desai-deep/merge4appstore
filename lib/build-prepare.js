...
export async function generateTestFlightNotes({ profile = null, build, payload, asc, github, signal = null }) {
  throwIfAborted(signal);
  const warnings = [];
  let publishedHistoryError = null;
  const publishedCommitsPromise = build.includeCommits
    ? asc.getPublishedWorkflowCommits(build.workflowId).catch(error => {
      throwIfAborted(signal);
...
export async function prepareBuild({ profile, build, payload, asc, github, signal = null }) {
  throwIfAborted(signal);
  ...
  const previousAscSignal = asc.signal;
  const previousGitHubSignal = github.signal;
  asc.signal = linked.controller.signal;
  github.signal = linked.controller.signal;
  let firstError = null;
  ...
  try {
    results = await Promise.allSettled([
      settle(asc.getAppStoreVersions()),
      settle(generateTestFlightNotes({
        profile,
        build,
        payload,
        asc,
    ...
    linked.dispose();
    asc.signal = previousAscSignal;
    github.signal = previousGitHubSignal;
  }
  if (firstError) throw firstError;
  const [versions, notes] = results.map(result => result.value);
  throwIfAborted(signal);
