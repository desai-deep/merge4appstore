...
export async function generateTestFlightNotes({ profile = null, build, payload, asc, github }) {
  const warnings = [];
  ...
  let pullRequest = payload.pull_request ? github.getPRDetails(payload.pull_request) : null;
  if (build.purpose === 'beta' && !pullRequest) {
    const betaBranch = profile?.repository?.beta_branch || 'develop';
    const productionBranch = profile?.repository?.production_branch || 'main';
    const releasePullRequest = github.findOpenPullRequestForCommit?.(
      payload.commit,
      productionBranch,
      betaBranch,
    );
    if (releasePullRequest?.number) {
      pullRequest = github.getPRDetails(releasePullRequest.number);
    }
  }
  ...
  if (build.includeCommits) {
    ...
    commitRange = github.getCommitSubjectsSince(publishedCommits, payload.commit);
