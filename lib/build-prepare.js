import { RELEASE_NOTES_FORMAT_MARKER } from './test-notes.js';
...
function releasePullRequestNotes(body) {
  ...
  const notes = section ? section[1].trim() : text;
  // V2 titles may themselves start with #number; only legacy lists need cleanup.
  if (!section || text.split(/\r?\n/).includes(RELEASE_NOTES_FORMAT_MARKER)) return notes;
  return notes.replace(/^(-[\t ]+)#\d+[\t ]+/gm, '$1');
}
