...
export function createWebhookServer({ secret, onXcodeCloud, log = console.log, maxBodyBytes = MAX_BODY_BYTES }) {
  return http.createServer(async (req, res) => {
    ...
    // Wrap in Promise.resolve so both sync throws and async rejections are
    // captured (the handler may be async).
    Promise.resolve()
      .then(() => onXcodeCloud(payload))
      .catch(e => log(`Error handling webhook trigger: ${e.message}`));
  });
}
