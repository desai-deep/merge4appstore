...
export class FileGitHubInstallationState {
  ...
  async initialize() {
    ...
    for (const entry of entries) {
      ...
      // Atomic writes may be in flight in another worker, or left behind by
      // a crash before rename. Only committed JSON files are authoritative;
      // never read or remove a temporary file that another worker may own.
      if (entry.isFile() && /^\.[1-9]\d*\.json-[0-9a-f]{8}(?:-[0-9a-f]{4}){3}-[0-9a-f]{12}\.tmp$/.test(entry.name)) continue;
