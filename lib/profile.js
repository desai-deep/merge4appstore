...
export function validateRepositoryProfile(profile) {
  ...
  if (profile.webhooks !== undefined) {
    ...
    for (const provider of Object.keys(profile.webhooks)) {
      ...
      if (settings.secret_env !== undefined) assertString(settings.secret_env, `webhooks.${provider}.secret_env`);
      if (settings.token_env !== undefined) assertString(settings.token_env, `webhooks.${provider}.token_env`);
    }
  }
