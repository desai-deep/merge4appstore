...
export const DEFAULT_RELEASE_PR_TITLE = 'Bug fixes and performance improvements';
...
export function validateRepositoryProfile(profile) {
  ...
  if (profile.repository.github_id !== undefined
    && (!Number.isSafeInteger(profile.repository.github_id) || profile.repository.github_id <= 0)) {
    throw new Error('repository.github_id must be a positive safe integer');
  }
  if (profile.repository.production_branch !== undefined) {
    assertString(profile.repository.production_branch, 'repository.production_branch');
  }
  if (profile.repository.beta_branch !== undefined) {
    assertString(profile.repository.beta_branch, 'repository.beta_branch');
  }

  resolveReleasePullRequest(profile);
  resolveAutoRebasePullRequests(profile);

  if (!profile.versioning || typeof profile.versioning !== 'object' || Array.isArray(profile.versioning)) {
    throw new Error('versioning must be a mapping');
  }
  for (const key of Object.keys(profile.versioning)) {
    if (key !== 'initial_version') throw new Error(`versioning.${key} is not supported`);
  }
  assertString(profile.versioning.initial_version, 'versioning.initial_version');
  if (!MARKETING_VERSION.test(profile.versioning.initial_version)) {
    throw new Error('versioning.initial_version must contain two or three numeric components');
  }
  ...
  if (profile.ci !== undefined) {
    ...
    for (const key of Object.keys(profile.ci)) {
      if (key !== 'version') throw new Error(`ci.${key} is not supported`);
    }
    if (profile.ci.version !== undefined) {
      if (!profile.ci.version || typeof profile.ci.version !== 'object' || Array.isArray(profile.ci.version)) {
        throw new Error('ci.version must be a mapping');
      }
      for (const key of Object.keys(profile.ci.version)) {
        if (key !== 'token_env') throw new Error(`ci.version.${key} is not supported`);
      }
      if (profile.ci.version.token_env !== undefined) assertString(profile.ci.version.token_env, 'ci.version.token_env');
    }
  }
  ...
  if (profile.build !== undefined) {
    ...
    for (const purpose of Object.keys(profile.build.purposes)) {
      const resolved = resolveBuildPurpose(profile, purpose);
      const existingPurpose = workflows.get(resolved.workflowId);
      if (existingPurpose) {
        throw new Error(
          `build purposes ${existingPurpose} and ${purpose} cannot share Xcode Cloud workflow ${resolved.workflowId}`,
        );
      }
      workflows.set(resolved.workflowId, purpose);
    }
  }
...
export function resolveReleasePullRequest(profile) {
  const configured = profile.release_pull_request;
  if (configured === undefined || configured === false) return { enabled: false };
  if (configured !== true && (!configured || typeof configured !== 'object' || Array.isArray(configured))) {
    throw new Error('release_pull_request must be a boolean or mapping');
  }

  const settings = configured === true ? {} : configured;
  for (const key of Object.keys(settings)) {
    if (!['enabled', 'title', 'note_limit'].includes(key)) {
      throw new Error(`release_pull_request.${key} is not supported`);
    }
  }
  if (settings.enabled === false) return { enabled: false };
  if (settings.enabled !== undefined && settings.enabled !== true) {
    throw new Error('release_pull_request.enabled must be a boolean');
  }
  if (settings.title !== undefined) assertString(settings.title, 'release_pull_request.title');
  if (settings.note_limit !== undefined
    && (!Number.isInteger(settings.note_limit) || settings.note_limit < 1)) {
    throw new Error('release_pull_request.note_limit must be a positive integer');
  }

  const baseBranch = profile.repository.production_branch || 'main';
  const headBranch = profile.repository.beta_branch || 'develop';
  if (baseBranch === headBranch) {
    throw new Error('release_pull_request requires different production and beta branches');
  }
  ...
  return {
    ...
    baseBranch,
    headBranch,
    title: settings.title || DEFAULT_RELEASE_PR_TITLE,
    noteLimit: settings.note_limit || 100,
  };
}

export function resolveAutoRebasePullRequests(profile) {
  const configured = profile.auto_rebase_pull_requests;
  if (configured !== undefined && typeof configured !== 'boolean') {
    throw new Error('auto_rebase_pull_requests must be a boolean');
  }

  return {
    enabled: configured === true,
    baseBranch: profile.repository.beta_branch || 'develop',
  };
}
...
export function applyRepositoryProfile(profile, environment = process.env) {
  ...
  if (profile.repository.github_id !== undefined) {
    const repositoryId = String(profile.repository.github_id);
    if (environment.GITHUB_REPOSITORY_ID !== undefined
      && environment.GITHUB_REPOSITORY_ID !== repositoryId) {
      throw new Error(
        `Webhook repository id ${environment.GITHUB_REPOSITORY_ID} does not match profile repository.github_id ${repositoryId}`,
      );
    }
    environment.GITHUB_REPOSITORY_ID = repositoryId;
  }
