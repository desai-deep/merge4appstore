...
function runtimeDirectory(kind) {
  const stateDirectory = process.env.MERGE4APPSTORE_STATE_DIR
    || path.join(os.homedir(), '.local', 'state', 'merge4appstore');
  return path.join(path.resolve(stateDirectory), kind);
}
...
export const CONFIG = {
  ...
  get logFile() { return path.join(runtimeDirectory('logs'), `${this.instanceName}.log`); },
  get lockFile() { return path.join(runtimeDirectory('jobs'), `${this.instanceName}.lock`); },
