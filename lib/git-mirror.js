...
export class GitMirror {
  ...
  async refresh({ headCommit = null, force = false, signal = null } = {}) {
    ...
    try {
      ...
      // Failures before its callback (state setup, lock acquisition, abandoned
      // clone cleanup) and before validation must therefore share the same
      // initialization backoff as clone/validation failures. Validated mirrors
      // retain refresh-only backoff so a transport outage does not make their
      // existing local data unavailable.
      if (!this.validated && !failure[initializationFailure]) {
        this.recordInitializationFailure(failure);
      }
      this.refreshRetryAt = this.now() + this.retryBackoffMs;
