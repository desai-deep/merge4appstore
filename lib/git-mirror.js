...
// A failed candidate clone must back off, not be mistaken for corruption in
// the active mirror and immediately cloned a second time in the same attempt.
const initializationFailure = Symbol('git-mirror-initialization-failure');
...
function normalizeRepositoryId(value) {
  ...
  if (
    ...
    throw new Error('Git mirror repository id must be a positive safe integer');
  }
  return normalized;
}
...
async function ensureGitSupportsNoLazyFetch() {
  if (!gitCapabilityCheck) {
    const { GH_TOKEN: _credential, ...capabilityEnvironment } = githubProcessEnvironment(process.env);
    gitCapabilityCheck = execFileAsync('git', ['--version'], {
      encoding: 'utf8',
      env: { ...capabilityEnvironment, LC_ALL: 'C' },
      timeout: 5_000,
      maxBuffer: 64 * 1024,
    }).then(({ stdout }) => {
...
function mirrorName(owner, repository, remoteUrl, repositoryIdentity) {
  const label = `${owner}-${repository}`.toLowerCase().replace(/[^a-z0-9._-]+/g, '-').slice(0, 80);
  const digest = crypto.createHash('sha256')
    .update(`${repositoryIdentity}\0${remoteUrl}`)
    .digest('hex')
    .slice(0, 12);
...
async function runGit(args, {
  ...
  if (!network) delete gitEnvironment.GH_TOKEN;
...
export class GitMirror {
  constructor(owner, repository, {
    ...
    this.repositoryId = normalizeRepositoryId(repositoryId);
    // Names are mutable and can be deleted/recreated. Once an immutable ID is
    // configured, use it for durable storage, locking, and metadata validation
    // so a replacement repository can never inherit the old repository's
    // objects merely because it reused the same owner/name and remote URL.
    this.repositoryIdentity = this.repositoryId
      ? `id:${this.repositoryId}`
      : this.identity;
    this.remoteUrl = validateRemoteUrl(remoteUrl);
    this.lockIdentity = `${this.repositoryIdentity}\0${this.remoteUrl}`;
  ...
  async git(args, {
    ...
    const suppliedEnvironment = network
      ? await this.environmentProvider({ signal })
      : this.environment;
    const environment = githubProcessEnvironment(suppliedEnvironment);
    if (!network) delete environment.GH_TOKEN;
  ...
  async validateMirror(target = this.mirrorPath, { signal = null } = {}) {
    ...
    if (identity.trim() !== this.repositoryIdentity) {
      throw new InvalidMirrorError('Mirror repository identity does not match');
    }
  ...
  async configureMirror(target, { signal = null } = {}) {
    await this.git([
      '-C', target, 'config', 'merge4appstore.repository', this.repositoryIdentity,
    ], { signal });
  }
  ...
  recordInitializationFailure(error) {
    ...
    if (!failure[initializationFailure]) {
      Object.defineProperty(failure, initializationFailure, { value: true });
    }
  ...
  async initializeMirrorLocked(signal = null) {
    ...
    try {
      ...
      ], { signal, timeoutMs: this.cloneTimeoutMs, network: true });
  ...
  async refresh({ headCommit = null, force = false, signal = null } = {}) {
    ...
    try {
      await this.withMutationLock('refresh', async operationSignal => {
        ...
        try {
          await this.cleanupAbandonedCloneDirectories();
          const initializing = !this.validated;
          ...
          try {
            ...
            // Cancellation is caller-owned state, not evidence that mirror
            // initialization is unhealthy. Preserve the exact abort reason and
            // leave retry state untouched for the next healthy request.
            if (operationSignal?.aborted) throw operationSignal.reason;
            if (isAbortLike(error)) throw error;
            ...
            if (!(error instanceof InvalidMirrorError)) {
              const failure = transientError(
                `Could not validate Git mirror for ${this.identity}`,
                error,
              );
              throw initializing ? this.recordInitializationFailure(failure) : failure;
            }
            this.validated = false;
            let initializedNow;
            try {
            ...
            if (initializedNow) {
              ...
              try {
                if (await this.hasCommitUnchecked(headCommit, { signal: operationSignal })) {
                  return;
                }
              } catch (error) {
                if (operationSignal?.aborted) throw operationSignal.reason;
          ...
          if (operationSignal?.aborted) throw operationSignal.reason;
          if (isAbortLike(error)) throw error;
          if (!error?.[initializationFailure] && isObjectCorruption(error)) {
            try {
              await this.rebuildCorruptMirrorLocked(operationSignal);
              return;
            } catch (repairError) {
          ...
          // Record initialization backoff before releasing the lock so a
          // forced caller queued behind a pre-validation failure cannot pass
          // the in-lock retry check. The outer catch covers failures that occur
          // before this callback starts.
          if (!this.validated && !failure[initializationFailure]) {
            this.recordInitializationFailure(failure);
          }
          // Set refresh backoff here for the same queued-caller guarantee.
      ...
      const failure = this.withConfiguredRetryAfter(error.statusCode === 503
        ? error
        : transientError(`Could not refresh Git mirror for ${this.identity}`, error));
      // A forced refresh performs first-use initialization under this lock.
      // Failures before its callback (state setup or lock acquisition) must
      // therefore share the same initialization backoff as failures recorded
      // inside the lock. Validated mirrors retain refresh-only backoff so a
      // transport outage does not make their existing local data unavailable.
      if (!this.validated && !failure[initializationFailure]) {
        this.recordInitializationFailure(failure);
      }
      this.refreshRetryAt = this.now() + this.retryBackoffMs;
...
export function getGitMirror(owner, repository, options = {}) {
  ...
  const repositoryId = normalizeRepositoryId(options.repositoryId);
  const repositoryScope = repositoryId
    ? `id:${repositoryId}`
    : `name:${owner.toLowerCase()}/${repository.toLowerCase()}`;
  const effectiveRemoteUrl = options.remoteUrl === undefined
    ? `https://github.com/${owner}/${repository}.git`
    : options.remoteUrl;
  const key = `${stateDirectory}\0${repositoryScope}\0${effectiveRemoteUrl}`;
  if (!mirrors.has(key)) {
    mirrors.set(key, new GitMirror(owner, repository, {
      ...options,
      stateDirectory,
      repositoryId,
    }));
  } else {
    // Keep the durable mirror object and its local caches, but never pin the
    // first caller's short-lived credential snapshot in the process registry.
    const mirror = mirrors.get(key);
    if (options.environmentProvider) mirror.environmentProvider = options.environmentProvider;
    if (options.environment) mirror.environment = options.environment;
  }
