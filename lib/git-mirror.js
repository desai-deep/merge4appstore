...
export class GitMirror {
  constructor(owner, repository, {
    ...
    lockTimeoutMs = positiveInteger(process.env.MERGE4APPSTORE_MIRROR_LOCK_TIMEOUT_MS, 60_000),
  ...
  async git(args, { signal = null } = {}) {
    return this.run(args, { timeoutMs: this.commandTimeoutMs, signal });
  }
  ...
  recordInitializationFailure(error) {
    const failure = error.statusCode === 503
      ? error
