...
import { extractTestNotes, RELEASE_NOTES_FORMAT_MARKER } from './test-notes.js';
...
export function releasePullRequestBody(pullRequests, {
  ...
  const notes = shown.length > 0
    ? shown.map(pull => {
      const testNotes = extractTestNotes(pull.body);
      const instructions = testNotes.split('\n').map(line => line ? `  ${line}` : '').join('\n');
      return `- ${pull.title}${testNotes ? `\n\n${instructions}` : ''}`;
    })
