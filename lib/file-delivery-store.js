...
function recoveryHistoryIsValid(history) {
  return Array.isArray(history)
    ...
    && history.every(entry => (
      ...
      && Number.isFinite(entry.requeuedAt)
      && entry.requeuedAt >= 0
    ));
}
...
export class FileDeliveryStore {
  constructor({
    ...
    this.retiredDirectory = path.join(this.receiptsDirectory, 'retired');
  ...
  async initialize() {
    ...
    // Retention maintenance is not part of the serving-readiness contract.
    // New PM2 generations start concurrently, and making every worker prune
    // before it can signal ready can exhaust the handoff timeout on a busy
    // state directory. Normal claim and recovery paths still run the bounded
    // maintenance pass after the worker is available.
  }
  ...
  stateDirectoryFor(state) {
    ...
    if (state === 'retired') return this.retiredDirectory;
  ...
  validateReceipt(receipt, file, hash) {
    ...
    if (!['pending', 'failed', 'complete', 'retired'].includes(receipt.state)) {
    ...
    if (
      receipt.recoveryHistory !== undefined
      && !recoveryHistoryIsValid(receipt.recoveryHistory)
    ) {
      throw this.corruptReceiptError(file, 'invalid recovery history');
    }
    if (receipt.state === 'retired' && receipt.retirement === undefined) {
      throw this.corruptReceiptError(file, 'missing retirement audit record');
    }
    if (receipt.retirement !== undefined) {
      const preservedLastError = typeof receipt.lastError === 'string' ? receipt.lastError : null;
      if (
        receipt.state !== 'retired'
        || !retirementAuditIsValid(receipt.retirement)
        || receipt.retirement.cursor > receipt.intent.jobs.length
        || receipt.retirement.retiredAt !== receipt.updatedAt
        || receipt.retirement.cursor !== receipt.cursor
        || receipt.retirement.attempts !== receipt.attempts
        || receipt.retirement.lastError !== preservedLastError
        || receipt.ownerPid != null
        || receipt.ownerIdentity != null
        || receipt.expiresAt !== undefined
      ) {
        throw this.corruptReceiptError(file, 'invalid retirement audit record');
      }
    }
    if (
      receipt.retryReason !== null
      && receipt.retryReason !== undefined
      && (typeof receipt.retryReason !== 'string' || !receipt.retryReason)
    ) {
      throw this.corruptReceiptError(file, 'invalid retry reason');
    }
    return receipt;
  }
  ...
  async resolveMultipleReceiptsLocked(hash, located) {
    ...
    if (
      ...
      && active.length === 1
      && complete
      && active[0].location !== 'retired'
      && active[0].receipt.state !== 'retired'
  ...
  async locateReceiptLocked(hash) {
    const located = [];
    for (const location of ['pending', 'failed', 'complete', 'retired']) {
  ...
  claimFor(hash, located) {
    return {
      ...
      ...(located.receipt.recoveryHistory
        ? { recoveryHistory: located.receipt.recoveryHistory.slice(-10) }
        : {}),
    };
  }
  ...
  receiptFromClaim(claim) {
    ...
    return {
      ...
      intent: claim.intent,
      ...(recoveryHistoryIsValid(claim.recoveryHistory)
        ? { recoveryHistory: claim.recoveryHistory.slice(-10) }
        : {}),
      updatedAt: this.now(),
    };
  }
  ...
  async takeOwnershipLocked(hash, {
    ...
    try {
      ...
      if (error.code !== 'ECORRUPTRECEIPT') throw error;
      try {
        // The retired directory is itself a fail-closed tombstone marker. Keep
        // provider redeliveries deduplicated even when its audit payload needs
        // repair; queue health still reports that corruption to operators.
        await fs.lstat(this.receiptFileForHash(hash, 'retired'));
        return null;
      } catch (retiredError) {
        if (retiredError.code !== 'ENOENT') throw retiredError;
        throw error;
      }
    }
    const now = this.now();
    ...
    if (current?.location === 'retired') return null;
    ...
    const releaseReason = releaseInstallationId === null
      ? null
      : `installation-suspended:${releaseInstallationId}`;
    if (
      respectRetryDelay
      && current?.location === 'pending'
      && Number(current.receipt.nextAttemptAt || 0) > now
      && !(releaseReason !== null && current.receipt.retryReason === releaseReason)
    ) return null;
    ...
    const updated = {
      ...
      ...(current?.location === 'pending' && current.receipt.recoveryHistory
        ? { recoveryHistory: current.receipt.recoveryHistory.slice(-10) }
        : {}),
  ...
  async retry(claim, error, { delayMs = 5_000, reason = null } = {}) {
    const hash = this.claimHash(claim);
    return this.withReceiptLock(hash, async () => {
      const { current, receipt } = await this.receiptForMutationLocked(hash, claim);
      if (!receipt) return false;
  ...
  async activeHashes() {
    return this.hashesIn([this.pendingDirectory, this.failedDirectory, this.retiredDirectory]);
  }
  ...
  async claimPending({ releaseInstallationId = null } = {}) {
    await this.ensureReady();
    await this.maybePrune();
    const claims = [];
    ...
    for (const hash of hashes) {
      let claim;
      try {
        claim = await this.takeOwnership(hash, { releaseInstallationId });
      } catch (error) {
  ...
  async queueStatus({ includeAge = false } = {}) {
    ...
    for (const hash of hashes) {
      const active = [];
      let corrupt = false;
      for (const location of ['pending', 'failed', 'retired']) {
        const file = this.receiptFileForHash(hash, location);
        try {
          const receipt = await this.readReceipt(file);
          if (receipt) {
            active.push({ location, receipt: this.validateReceipt(receipt, file, hash) });
          }
        } catch (error) {
          if (error.code !== 'ECORRUPTRECEIPT') throw error;
          corrupt = true;
      ...
      // Read complete/ only when a tombstone is involved. This detects a
      // duplicate retired/completed state without scanning the completed
      // backlog—or even probing it for ordinary pending and failed receipts.
      if (
        !corrupt
        && active.some(item => item.location === 'retired' || item.receipt.state === 'retired')
      ) {
        const file = this.receiptFileForHash(hash, 'complete');
        try {
          const receipt = await this.readReceipt(file);
          if (receipt) {
            active.push({ location: 'complete', receipt: this.validateReceipt(receipt, file, hash) });
          }
        } catch (error) {
          if (error.code !== 'ECORRUPTRECEIPT') throw error;
          corrupt = true;
      ...
      if (corrupt || active.length > 1) {
        status.failed += 1;
        status.corrupt += 1;
      } else if (active[0]?.receipt.state === 'pending') {
        status.pending += 1;
        oldestPendingUpdatedAt = oldestPendingUpdatedAt === null
          ? active[0].receipt.updatedAt
          : Math.min(oldestPendingUpdatedAt, active[0].receipt.updatedAt);
      }
      else if (active[0]?.receipt.state === 'failed') status.failed += 1;
    }
    if (includeAge) {
  ...
  async inspectFailedReceipts() {
    ...
    const failed = [];
    const corrupt = [];
    const hashes = [...await this.activeHashes()].sort();
    for (const hash of hashes) {
      await this.withReceiptLock(hash, async () => {
        const active = [];
        try {
          // Inspect complete/ for these active hashes too. This does not scan
          // the completed backlog, but it prevents a duplicate state file from
          // surfacing only after an earlier receipt in the batch was mutated.
          for (const location of ['pending', 'failed', 'complete', 'retired']) {
            const file = this.receiptFileForHash(hash, location);
            const receipt = await this.readReceipt(file);
            if (receipt) {
              active.push({
                location,
                receipt: this.validateReceipt(receipt, file, hash),
              });
            }
          }
        } catch (error) {
        ...
        const receipt = active[0]?.receipt;
        if (receipt?.state !== 'failed') return;
        const updatedAt = new Date(receipt.updatedAt);
        failed.push({
          hash,
          revision: deliveryReceiptRevision(hash, receipt),
          instance: receipt.intent.instance,
  ...
  async requeueFailed({ receiptHashes = null, expectedCount = null } = {}) {
    ...
    return this.withProcessLock(MANUAL_OPERATION_LOCK, async () => {
      const inspection = await this.inspectFailedReceipts();
      if (inspection.corrupt.length > 0) {
        const error = new Error(
          `Cannot requeue while ${inspection.corrupt.length} corrupt receipt hash(es) remain`,
        );
        error.code = 'ECORRUPTRECEIPT';
        throw error;
      }
      if (expectedCount !== null && inspection.failed.length !== expectedCount) {
        const error = new Error(
          `Failed receipt count changed: expected ${expectedCount}, found ${inspection.failed.length}`,
        );
        error.code = 'ECOUNTMISMATCH';
        throw error;
      }
      const available = new Set(inspection.failed.map(receipt => receipt.hash));
      const selected = requested === null
        ? [...available].sort()
        : [...requested].sort();
      const unavailable = selected.filter(hash => !available.has(hash));
      ...
      for (const hash of selected) {
        const didRequeue = await this.withReceiptLock(hash, async () => {
          ...
          const now = this.now();
          const history = Array.isArray(current.receipt.recoveryHistory)
            ? current.receipt.recoveryHistory.slice(-9)
            : [];
          const updated = {
            ...current.receipt,
            ...
            recoveryHistory: [...history, {
              failedAt: current.receipt.updatedAt,
              attempts: current.receipt.attempts,
              lastError: typeof current.receipt.lastError === 'string'
                ? current.receipt.lastError
                : null,
              requeuedAt: now,
            }],
            updatedAt: now,
  ...
  async retireFailed({
    ...
    await this.ensureReady();
    const hash = String(selectedHash || '').toLowerCase();
    if (!/^[0-9a-f]{64}$/.test(hash)) {
      throw new Error('Failed receipt selector must be a 64-character hexadecimal hash');
    }
    const reviewedRevision = String(revision || '').toLowerCase();
    if (!/^[0-9a-f]{64}$/.test(reviewedRevision)) {
      throw new Error('Receipt revision must be a 64-character hexadecimal hash');
    }
    const retirementReasonCode = normalizeRetirementReasonCode(reasonCode);
    const retirementReference = normalizeRetirementReference(reference);
    const retirementActor = normalizeRetirementActor(actor);

    return this.withProcessLock(
      MANUAL_OPERATION_LOCK,
      async () => this.withReceiptLock(hash, async () => {
        const current = await this.locateReceiptLocked(hash);
        if (current?.location !== 'failed') {
          const error = new Error(`Receipt is no longer a readable failed delivery: ${hash}`);
          error.code = 'ENOTFAILED';
          throw error;
        }
        const currentRevision = deliveryReceiptRevision(hash, current.receipt);
        if (currentRevision !== reviewedRevision) {
          const error = new Error(`Receipt changed after inspection: ${hash}`);
          error.code = 'ESTALEREVISION';
          throw error;
        }

        const now = this.now();
        const { expiresAt: _expiresAt, ...receipt } = current.receipt;
        const updated = {
          ...receipt,
          version: 2,
          receiptHash: hash,
        ...
        // Move the still-failed record first so the directory remains a durable
        // retirement-intent marker even if the subsequent rewrite is interrupted
        // or later corrupted. Before the atomic rewrite, ordinary lookup can
        // repair the failed receipt to failed/. After it, the tombstone is final.
        const retiredFile = await this.moveReceiptLocked(
          hash,
          current.location,
          'retired',
          current.receipt,
        );
        await this.writeReceipt(retiredFile, updated);
        return true;
      }),
    );
  }
    ...
    return this.withProcessLock(MANUAL_OPERATION_LOCK, async () => {
      ...
      for (const hash of hashes) {
        await this.withReceiptLock(hash, async () => {
          ...
          const retirementInvolved = [...invalid, ...valid].some(item => (
            item.location === 'retired' || item.receipt?.state === 'retired'
          ));
          // Never remove a tombstone: doing so would allow a late provider
          // redelivery to execute work that an operator explicitly retired.
          if (retirementInvolved) return;
          if (valid.length > 1) {
            try {
