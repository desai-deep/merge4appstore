...
export async function verifyGitHubAppInstallation(authenticator, owner, repository, {
  ...
  const [{ data: repositoryData }, { data: pulls }] = await Promise.all([
    authenticator.request(repositoryPath, { token: credential.token }),
    authenticator.request(`${repositoryPath}/pulls?state=all&per_page=1`, { token: credential.token }),
  ]);
  ...
  if (writeTag) {
    ...
    const tagName = `merge4appstore-app-verification-${now()}`;
