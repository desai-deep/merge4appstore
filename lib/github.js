  ...
  getCommitSubjectsSince(publishedCommits, headCommit, candidateLimit = 20) {
    const candidates = publishedCommits
      .map(candidate => typeof candidate === 'string'
        ? { commitSha: candidate, buildNumber: null, marketingVersion: null }
        : {
          commitSha: candidate?.commitSha,
          buildNumber: candidate?.buildNumber || null,
          marketingVersion: candidate?.marketingVersion || null,
        })
    ...
    for (const candidate of candidates) {
      const {
        ...
        marketingVersion: baseMarketingVersion,
      } = candidate;
      try {
        const result = this.exec([
          'api', '--paginate', '--slurp',
        ...
        const subjects = pages.flatMap(page => page.commits || [])
          ...
          .filter(Boolean);
        return { baseCommit, baseBuildNumber, baseMarketingVersion, subjects };
      } catch (e) {
