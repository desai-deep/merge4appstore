...
export class GitHubAPI {
  constructor(repoOwner, repoName, productionBranch = 'main', {
    ...
    // Long-running production callers pass the strict provider explicitly so
    // App tokens can rotate. Keep direct library construction usable for
    // synchronous/PAT-only clients and hermetic local mirror tests.
    this.environmentProvider = environmentProvider || (async () => ({ ...environment }));
    this.repositoryId = repositoryId;
    this.runSync = runSync;
    this.runAsync = runAsync;
    this.mirror = mirror === undefined ? getGitMirror(repoOwner, repoName, {
      environment,
  ...
  async requestJson(endpoint, { timeoutMs = 15_000 } = {}) {
    ...
    try {
      const environment = await this.refreshEnvironment({ signal: controller.signal });
  ...
  async refreshEnvironment({ signal = this.signal } = {}) {
  ...
  async execAsync(args) {
    ...
    const { stdout } = await this.runAsync('gh', args, {
      encoding: 'utf8',
      timeout: 15_000,
  ...
  getBranchSnapshot(branch) {
    ...
    return {
      ...
      committedAt: result.commit?.committer?.date || null,
    };
  }
  ...
  listMergedPullRequests(baseBranch) {
    const result = this.exec([
      ...
      '--base', baseBranch,
      '--limit', '1000',
      '--json', 'number,title,body,url,mergeCommit,mergedAt',
    ]);
    ...
    try {
      if (!headBranch) {
        ...
        const matches = commitPulls.filter(candidate => (
          !baseBranch || candidate.base?.ref === baseBranch
        ));

        // An exact commit may be associated with multiple PRs. Expire only
        // when one closed PR in the requested scope is unambiguous.
      ...
      if (baseBranch) args.splice(6, 0, '--base', baseBranch);
      const result = this.exec(args);
      const pulls = JSON.parse(result || '[]');
      const matches = pulls.filter(candidate => (
        (!baseBranch || candidate.baseRefName === baseBranch)
        && candidate.headRefName === headBranch
      ));
