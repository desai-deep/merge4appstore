  ...
  getCommitSubjectsSince(publishedCommits, headCommit, candidateLimit = 20) {
    const candidates = publishedCommits
      .map(candidate => typeof candidate === 'string'
        ? { commitSha: candidate, buildNumber: null }
        : { commitSha: candidate?.commitSha, buildNumber: candidate?.buildNumber || null })
      .filter(candidate => candidate.commitSha && candidate.commitSha !== headCommit)
      .slice(0, candidateLimit);

    for (const candidate of candidates) {
      const { commitSha: baseCommit, buildNumber: baseBuildNumber } = candidate;
      try {
        const result = this.exec([
          'api', '--paginate', '--slurp',
        ...
        const subjects = pages.flatMap(page => page.commits || [])
          ...
          .filter(Boolean);
        return { baseCommit, baseBuildNumber, subjects };
      } catch (e) {
